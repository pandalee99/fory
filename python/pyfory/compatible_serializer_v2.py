...
class ClassDef:
    ...
    def _calculate_type_hash(cls, type_cls: Type, field_infos: List[FieldInfo]) -> int:
        ...
        # Use MD5 hash and convert to int32 (like Java hashCode)
...
class MetaContext:
    ...
    def register_class_def(self, type_cls: Type) -> int:
...
class CompatibleSerializer(CrossLanguageCompatibleSerializer):
    ...
    def _write_embedded_field(self, buffer: Buffer, field_info: FieldInfo, field_value: Any):
        ...
        if field_value is None:
            ...
            if field_info.embedded_class_id == TypeId.INT64:
                ...
                # Fallback for unknown embedded types
                self.fory.serialize_nonref(buffer, field_value)
    ...
    def _write_separate_field(self, buffer: Buffer, field_info: FieldInfo, field_value: Any, xlang: bool):
        ...
        # Write the value using standard serialization
        if xlang:
    ...
    def _read_compatible(self, buffer: Buffer, xlang: bool) -> Any:
        ...
        for _ in range(embedded_count):
            ...
            if is_null:
                ...
                if field_info and field_info.embedded_class_id >= 0:
                    ...
                    # For now, we'll read it as a generic value
                    embedded_values[field_name] = self.fory.deserialize_nonref(buffer)
        ...
        for _ in range(separate_count):
            ...
            field_info = self._find_field_by_name(field_name)
            if field_info:
                # Read known separate field
                if xlang:
                    separate_values[field_name] = self.fory.xdeserialize_ref(buffer)
                else:
                    separate_values[field_name] = self.fory.deserialize_ref(buffer)
            else:
                # Skip unknown separate field
                logger.debug(f"Skipping unknown separate field: {field_name}")
                if xlang:
                    self.fory.xdeserialize_ref(buffer)  # Read and discard
                else:
                    self.fory.deserialize_ref(buffer)   # Read and discard
